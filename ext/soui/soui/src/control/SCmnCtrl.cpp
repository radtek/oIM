...
SToggle::SToggle()
{
    m_pBgSkin = GETBUILTINSKIN(SKIN_SYS_TREE_TOGGLE);
}
...
void SToggle::OnPaint(IRenderTarget *pRT)
{
    ...
    if(GetToggle()) nState+=3;
    m_pBgSkin->Draw(pRT, rtClient, nState);
    __super::OnPaint(pRT);
}
...
HRESULT SToggle::OnAttrCheck( const SStringW& strValue, BOOL bLoading )
{
    SetCheck(strValue != L"0");
    return S_FALSE;
}
...
void SToggle::OnStateChanged( DWORD dwOldState,DWORD dwNewState )
{
    __super::OnStateChanged(dwOldState,dwNewState);
}

void SToggle::OnSize( UINT nType, CSize size )
{
    __super::OnSize(nType,size);
}
