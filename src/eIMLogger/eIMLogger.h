...
class C_EIMLogger: public I_EIMLogger
{
private:
	ULONG		m_ulRef;		// Reference count of Cmd
	typedef enum tagEIMDateTimeFmt
	{
	...
	}E_EIMDateTimeFmt, *PE_EIMDateTimeFmt;

	typedef std::list<PS_EIMLogMsg> ListLogMsg;
	FILE*			m_fileLog;
	HANDLE			m_hLogThread;
	HANDLE			m_hLogEvent;
	HANDLE			m_hExitEvent;
...
public:
	...
	virtual bool Init(	// Initial log object
		...
		E_EIMLogLevel	eLogLevel   = EIMLOG_LEVEL_DEFAULT,	// Log level
		DWORD			dwLogFlag   = EIMLOG_FLAG_DEFAULT,	// Log Flag
	...
	virtual E_EIMLogLevel GetLogLevel();	// Log level, reference E_LogLevel
	virtual const TCHAR* GetLogFile();		// Log file

public:
	//Log function: input variable parameter 
	virtual void Log  (E_EIMLogLevel eLogLevel, const TCHAR* pszFormat, ...);
