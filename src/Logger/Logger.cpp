...
#include <AtlConv.h>
#include <Lmcons.h>
#include "public\ThreadUtil.h"

#define IS_APPEND(FLAG)				(FLAG & LOG_FLAG_APPEND)
...
#ifndef _NO_CREATEINTERFACE_API_
// Define the PLUGIN_API_ macro function to declare the standard API
extern "C" __declspec(dllexport) int __stdcall CreateInterface(const TCHAR* pctszIID, void** ppvIObject)
{
	...
	if( _tcsnicmp(pctszIID, INAME_LOGGER, _tcslen(INAME_LOGGER)) == 0 )
	{
...
{	
	if( eLogLevel < m_eLogLevel ||									// Skip this log message
		m_dwLogFlag == LOG_FLAG_NOINIT ||						// Not initial or uninited
		!(m_dwLogFlag & LOG_FLAG_WORKING))						// Log thread not working
		return;	

	S_LogMsg	sLogMsg;											// Log message information, temporary
	...
	if(m_dwLogFlag & LOG_FLAG_OUT_DEBUGVIEW)
...
//              
//=============================================================================
TCHAR* C_Logger::_GetLevelString(E_LogLevel eLogLeve)
{
	switch(eLogLeve)
	{
	case eLLDATA:		
...
TCHAR* C_Logger::_GetDateTimeString(
	TCHAR*			pszMsgBuf, 
	int&			i32Size, 
	SYSTEMTIME&		sTime,
...
//=============================================================================
TCHAR* C_Logger::_GetLogHeadInfo(
	TCHAR*		pszMsgBuf, 
	int&		i32Size, 
	E_LogLevel	eLogLeve,
	DWORD		dwTID, 
...
#define CHECK_RET(POS,RET) \
	...
	int i32Ret = 0;
	int i32Pos = 0;
	if(m_dwLogFlag & LOG_FLAG_HEAD_PID)
	...
	CHECK_RET(i32Pos, i32Ret);
	i32Ret =  i32Size - i32Pos;
	_GetDateTimeString(&pszMsgBuf[i32Pos], i32Ret, *psTime, (m_dwLogFlag & LOG_FLAG_HEAD_MS) ? eDTTIME_MS : eDTTIME, NULL, _T("]"));

	CHECK_RET(i32Pos, i32Ret);
	if(m_dwLogFlag & LOG_FLAG_HEAD_LEVEL)
...
//              
//=============================================================================
unsigned __stdcall C_Logger::_LogThreadProc(void* pParam)
{
	C_Logger* pLogger = static_cast<C_Logger*>(pParam);
	if(pLogger)
...
unsigned C_Logger::_DoLog()
{
	...
	while(true)
	{
		...
		case WAIT_OBJECT_0 + 1:
			...
			if((m_dwLogFlag & LOG_FLAG_OUT_FILE) && m_fileLog)
			{
			...
			{
				PS_LogMsg psECLogMsg = lstLogMsg.front();
				...
				int i32MsgSize = psECLogMsg->dwSize - sizeof(S_LogMsg);
				if((m_dwLogFlag & LOG_FLAG_OUT_FILE) && m_fileLog)
				{
					...
					if(i64LogFileSize > 0xFFFFFFFFll)
					{
						...
						{	// Can not log to file, redirect to DebugView
							m_dwLogFlag &= ~LOG_FLAG_OUT_FILE;	// Clear ECLOG_FLAG_OUT_FILE
							m_dwLogFlag |= LOG_FLAG_OUT_DEBUGVIEW;// Set ECLOG_FLAG_OUT_DEBUGVIEW
						}
					}
					else
						fwrite(psECLogMsg->szMsg, 1, i32MsgSize, m_fileLog);
...
//Del 				//============ End ===== 2013/11/27 15:52:37 deleted by lwq ======
				...
				if(m_dwLogFlag & LOG_FLAG_OUT_HWND)
					SendMessage( HWND_BROADCAST, m_dwWindowMsg, 0, (LPARAM)psECLogMsg);

				if(m_dwLogFlag & LOG_FLAG_OUT_STDOUT)
	...
	)
{
	...
	if(lpszLogFile)
		dwLogFlag |= LOG_FLAG_OUT_FILE;

	if(dwLogFlag & LOG_FLAG_OUT_FILE)
	{
	...
	if(dwLogFlag & LOG_FLAG_OUT_HWND)
		m_dwWindowMsg = RegisterWindowMessage(LOG_WINDOW_MSG_NAME);

	m_dwPID		= GetCurrentProcessId();					// Get current process Id
	m_eLogLevel = eLogLevel;								// Set log level
	m_dwLogFlag = dwLogFlag | LOG_FLAG_WORKING;			// Set log flag
...
// Log property 
bool C_Logger::IsInited()
{
	return (m_dwLogFlag != LOG_FLAG_NOINIT);
}

int C_Logger::GetLogFlag()
{
	return m_dwLogFlag;
}

E_LogLevel C_Logger::GetLogLevel()
{
	return m_eLogLevel;
}

const TCHAR* C_Logger::GetLogFile()
{
	return m_szLogFile;
}

//Log function: input variable parameter 
void C_Logger::Log(E_LogLevel eLogLevel, const TCHAR* pszFormat, ...)
{
...
void C_Logger::Data(const BYTE* pu8Data, int i32Size, const TCHAR* pszFormat, ...)
{
...
void C_Logger::Debug(const TCHAR* pszFormat, ...)
{
...
void C_Logger::Trace(const TCHAR* pszFormat, ...)
{
...
void C_Logger::Info(const TCHAR* pszFormat, ...)
{
...
void C_Logger::Warn(const TCHAR* pszFormat, ...)
{
...
void C_Logger::Error(const TCHAR* pszFormat, ...)
{
...
void C_Logger::Fatal(const TCHAR* pszFormat, ...)
{
	va_list args;
	va_start(args, pszFormat);
