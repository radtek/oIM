...
int CLIENT_ntoh_cmd(PCONNCB pConnCB, int &nContentLength, int &nMsgNotice, TERM_CMD_HEAD **ppHead)
{
	...
	//设备管理应答
	case CMD_MANAGERDEVICE_ACK:
		{
			MANAGERDEVICEACK *pAck = (MANAGERDEVICEACK*)pHead->aszMsg;
			ntoh_term_managerdevicAck(pAck);
			break;
		}
    //用户修改信息通知
...
#endif
{
	...
	if(pConnCB != NULL)
	{
//		pConnCB->pLog->PrintLog(DEBUG_LEVEL, "connect succuss,socket:%d, ip:%s",(unsigned int)(pConnCB->nSocket),aszIP);
        pConnCB->pLog->PrintLog(DEBUG_LEVEL, "connect succuss,socket:%d, ip:",(unsigned int)pConnCB->nSocket);
    }
	#endif
...
#endif
{
	...
	if (tmpLen >= PASSWD_MAXLEN)
		tmpLen = PASSWD_MAXLEN - 1;
...
#ifdef WIN32
IM_API( int )  CLIENT_ManagerDevice(PCONNCB pConnCB, MANAGERDEVICEREQ *pReqInfo)
#else
int   CLIENT_ManagerDevice(PCONNCB pConnCB, MANAGERDEVICEREQ *pReqInfo)
#endif
{
    MANAGERDEVICEREQ *pManagerDeviceReq = NULL;
    EMPLOYEE *pEmpy;

    int nCmdLen = 0;
    int nRet = 0;
    char aszPacket[PACKET_MAXLEN];
    int nMsgLen = sizeof(MANAGERDEVICEREQ) + MSGHEAD_LEN;
    ...
    TERM_CMD_HEAD *pHead = (TERM_CMD_HEAD*)(aszPacket + nCmdLen);
    pManagerDeviceReq = (MANAGERDEVICEREQ*)((char*)pHead + MSGHEAD_LEN);

    memcpy(pManagerDeviceReq, pReqInfo, sizeof(MANAGERDEVICEREQ));
    hton_term_managerdevice_req(pManagerDeviceReq);


    pHead->wCmdID = CMD_MANAGERDEVICE_REQ;
    pHead->dwSeq = g_nSeq++;
    pHead->wMsgLen = nMsgLen;
