...
//祥源需求 登录账户64位 密码最长32位
#if defined(_XIANGYUAN_FLAG_)

#define _ACCOUNT_64_FLAG_
#define _PASSWD_32_FLAG_
...
#elif defined _BIGUIYUAN_FLAG_ 
#define PASSWD_MAXLEN       51

#elif defined _PASSWD_32_FLAG_
...
#define USERCODE_LEN                    30          //工号长度
#define IMEI_LEN						50			//手机唯一标识符长度
#define PRODUCT_LEN						32			//手机型号
#define VERSION_LEN						32			//手机版本长度
...
typedef enum
{
	...
	CMD_MANAGERDEVICE_REQ = 221,		//设备管理请求
	CMD_MANAGERDEVICE_ACK = 222,		//设备管理应答
} TERM_CMD_TYPE; 
...
typedef struct _Login
{
	...
	LV64 tAccount;	// 登录帐号
#elif defined (_ACCOUNT_64_FLAG_)
    LV64 tAccount;	// 登录帐号
#else
	LV32 tAccount;	// 登录帐号
#endif
...
{
	UINT8  bAllStatus;		// 非零时，时全量请求的应答（无法区分，就无法让客户端正常处理全量）
...
typedef struct _ManagerDeviceRsp{
	UINT32	state;  					//设备状态   0.正常 1.禁用
	UINT32	result;						//请求处理成功与否	0.成功 -1.失败
	INT8	IMEI[IMEI_LEN];				//手机唯一标识符
}MANAGERDEVICEACK;
...
// 设备管理请求
typedef struct _manager_device_req{
	INT8    aszUserCode[USERCODE_LEN];				//工号
	UINT32	userId;	  								//用户id
	INT8	IMEI[IMEI_LEN];							//手机唯一标识符
	UINT32	terminal_type;   						//终端类型1 安卓 0 ios 终端类型
	INT8	product[PRODUCT_LEN];					//手机品牌
	INT8	version[VERSION_LEN];					//手机系统版本
}MANAGERDEVICEREQ;
...
#if defined(_GOME_FLAG_)
	LV64  tUserAccount ;//用户帐号LV方式；1字节长度，帐号最长29字节
#elif defined(_ACCOUNT_64_FLAG_)
    LV64  tUserAccount ;//用户帐号LV方式；1字节长度，帐号最长29字节
#else
