...
void ntoh_term_managerdevicAck(MANAGERDEVICEACK *p)
{
	p->result = ntohl(p->result);
	p->state = ntohl(p->state);
}
...
void hton_term_managerdevice_req(MANAGERDEVICEREQ *p)
{
	p->userId = htonl(p->userId);
	p->terminal_type =  htonl(p->terminal_type);
}
