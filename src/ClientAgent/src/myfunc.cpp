...
{	// The length in pszData is DECIMAL ASCII
	...
	#ifdef _GOME_FLAG_ 
		{
			pData->value[0] = 'B';//xie yi ban ben hao
			memcpy(pData->value+1,pThis->aszVersion,sizeof(pThis->aszVersion)-1);
	...
	int toDecodeTALLUserStatus(TALLUserStatus*pThis, UINT32 uBegID,UINT32 uGetNum,TGetStatusRsp* pData,UINT32* pEndUserID  )
	{
		...
		for(int i = (uBegID - pThis->uBegUserID) / 4; i < pThis->MAX_BYTE_BITMAP; i++)
		{
			for(int j=0; j < 4; j++)
			{
				...
				USERSTATUSNOTICE& sUser = pData->szUserStatus[pData->dwUserStatusNum];
				sUser.cStatus=cStatus;
				if (cStatus == 3)
				{
					sUser.cLoginType = 1; //移动在线
					sUser.cStatus = 1;
				}
				else if (cStatus != 0)
					sUser.cLoginType = 3; //PC在线或离开

				if (cStatus != 0)
				{
					sUser.dwUserID=pThis->uBegUserID+ 4*i +j;
