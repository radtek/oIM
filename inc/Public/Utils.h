...
#ifndef __EIM_UTIL_API_2013_9_9_BY_YFGZ__
#define __EIM_UTIL_API_2013_9_9_BY_YFGZ__
...
#define CHECK_NULL_RET_(P, R)		do { if( !(P) ) { TRACE_(_T("Parameter ") _T(#P) _T(" is NULL")); return (R); }}while(0)
#define CHECK_NULL_RETV_(P)			do { if( !(P) ) { TRACE_(_T("Parameter ") _T(#P) _T(" is NULL")); return; }}while(0)
#define CHECK_FAILED_RET_(R,RET)	do { if( FAILED(R) ) { TRACE_(_T("Result: 0x%08X"), (R)); return (RET); } }while(0)
...
const char* eIMCheckColTextNull(const unsigned char* colText);
int	eIMCompareVersion(const TCHAR* pszVerA, const TCHAR* pszVerB);      // Compare version string 
...
#endif	// __EIM_UTIL_API_2013_9_9_BY_YFGZ__
