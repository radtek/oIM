...
private:
    bool LoadDll() throw()
    {
		...
		eIMString szLib = PATH_TOKEN_EXE_FILE_(_T("crshhndl.dll"));
		::eIMReplaceToken(szLib);

        HMODULE hCrshhndlDll = ::LoadLibrary(szLib.c_str());
		if ( hCrshhndlDll == NULL )
			hCrshhndlDll = ::LoadLibrary(_T("crshhndl.dll"));
